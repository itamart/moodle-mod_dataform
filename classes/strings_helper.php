 ...
 */
class mod_dataform_strings_helper {

    /**
     * Returns list of all Dataform strings.
     *
     * @return array
     */
    public static function all() {
        ...
        foreach (self::get_string_categories() as $category) {
            $string = array_merge($string,  self::$category());
        }
     ...
     * @return array
     */
    protected static function get_string_categories() {
        return array(
            'general',
            'event',
            'capability',
    ...
    /**
     * Returns array of strings Dataform related strings .
     *
     * @return array
     */
    ...
    /**
     * Returns array of strings related to dataform events.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform events.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform messages and notifications.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform views.
     *
     * @return array
     */
    ...
    /**
     * Returns list of Dataform field related strings.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform filters.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform rules.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform tools.
     *
     * @return array
     */
    ...
    /**
     * Return array of strings related to dataform presets.
     *
     * @return array
     */
    ...
    /**
     * Returns summary of dataform strings status.
     *
    ...
    public static function get_summary() {
        ...
        $stringsmenu = self::all();
        $strings = array_keys($stringsmenu);
        $found = array();
        $notfound = array();
        
        $getstring = "get_string\(['\"]([^'\"]+)['\"],\s*'(mod_dataform|dataform)'";
        
        // Iterate all php files under mod/dataform
        foreach (get_directory_list("$CFG->dirroot/mod/dataform/") as $filename) {
            if (substr($filename, -4) != '.php') {
                continue;
            }
            // Get file content
            if (!$content = file_get_contents("$CFG->dirroot/mod/dataform/$filename")) {
                continue;
            }
            // Search stings and remove from list when found
            
            if (preg_match_all("%$getstring%", $content, $matches)) {
                foreach ($matches[1] as $match) {
                    if (!in_array($match, $strings)) {
                        $notfound[$match] = $filename;
                    } else {
                        $found[$match] = $match;
                    }
                }
        ...
        $notfoundlist = new \html_table();
        $notfoundlist->head = array('', get_string('name'), get_string('file'));
        
        $i = 1;
        foreach ($notfound as $string => $location) {
            $notfoundlist->data[] = array(
                $i++,
                $string,
                $location,
            );
        ...
        // Redundant strings
        $diff = array_diff_key($stringsmenu, $found);
        $numbering = function($a) {
            static $i = 0;
            $i++;
            return "$i. $a";
        };
        $diff = array_map($numbering, array_keys($diff));
        
        $html = \html_writer::tag('h3', 'Unidentified strings');
        $html .= \html_writer::table($notfoundlist);;
        $html .= \html_writer::tag('h3', 'Uncalled strings');
        $html .= implode('<br />', $diff);
