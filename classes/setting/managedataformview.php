    ...
    public function output_html($data, $query = '') {
        ...
        foreach ($allplugins as $plugin => $unused) {
            ...
            if (isset($enabled[$plugin])) {
                ...
                $hideshow .= $OUTPUT->pix_icon('t/hide', '', 'moodle', array('class' => 'iconsmall', 'alt' => $strdisable)). '</a>';
                $isenabled = true;
                ...
                if (isset($available[$plugin])) {
                    ...
                    $hideshow = "<a href=\"$aurl\">";
                    $hideshow .= $OUTPUT->pix_icon('t/show', '', 'moodle', array('class' => 'iconsmall', 'alt' => $strenable)). '</a>';
                    $isenabled = false;
            ...
            if ($isenabled) {
                if ($updowncount > 1) {
                    ...
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= $OUTPUT->pix_icon('t/up', '', 'moodle', array('class' => 'iconsmall', 'alt' => $strup)). '</a>';
                } else {
                    $updown .= $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'iconsmall', 'alt' => '')). '&nbsp';
                }
                if ($updowncount < $plugincount) {
                    $aurl = new \moodle_url($url, array('action' => 'down', 'plugin' => $plugintype));
                    $updown .= "<a href=\"$aurl\">";
                    $updown .= $OUTPUT->pix_icon('t/down', '', 'moodle', array('class' => 'iconsmall', 'alt' => $strdown)). '</a>';
                } else {
                    $updown .= $OUTPUT->pix_icon('spacer', '', 'moodle', array('class' => 'iconsmall', 'alt' => ''));
                }
                ++$updowncount;
            }
