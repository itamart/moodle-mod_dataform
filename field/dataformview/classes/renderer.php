...
class dataformfield_dataformview_renderer extends mod_dataform\pluginbase\dataformfieldrenderer {
     ...
     *
     */
    public function display_browse($entry, $type = null) {
        global $PAGE;
        
        $field = $this->_field;

        if (!$field->refdataform or !$field->refview) {
            return '';
        }
    ...
    protected function get_view_display_content($entry, array $options = array()) {
        ...
        $localview = $field->local_view;

        // Options for setting the filter
        $foptions = array();
        // Filter id
        if ($field->reffilterid) {
            $foptions['id'] = $field->param3;
        }
        ...
        $refview->set_viewfilter($foptions);
    ...
    protected function get_filter_by_options(array $options, $entry, $urlquery = false) {
        ...
        // For new entries take current user
        if ($field->param6) {
            list($filterauthor, $filtergroup) = explode(',', $field->param6);
            // Entry author
            if ($filterauthor) {
    ...
    protected function get_search_options($entry) {
        ...
        if (!$refdataform = $field->refdataform or !$refview = $field->refview or !$localview = $field->local_view) {
            //return $soptions;
        }
        ...
        foreach ($searchoptions as $searchy) {
            ...
            if (!$localfieldid = $localview->get_pattern_fieldid($localpattern)) {
                $value = $localpattern;
            } else if ($localfield = $field->df->field_manager->get_field_by_id($localfieldid)) {
